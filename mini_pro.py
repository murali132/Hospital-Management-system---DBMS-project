...
# creating the frames in the window
        ...
        self.averageage = Frame(self.window)
        self.subqueryframe= Frame(self.window)
        self.addqueryframe= Frame(self.window)
...
# funtion to call submit button
    ...
    def homee(self):
        ...
        self.averageage.destroy()
        self.subqueryframe.destroy()
        self.addqueryframe.destroy()
...
#buttons for update and delete
    ...
    def averageAgee(self):
        ...
        self.id = Label(self.averageage, text="Average Age", font=('arial 12 bold'), fg="red")
        self.id.place(x=0, y=12)
        
        self.search = Button(self.averageage, text="GetAvgAge", font="aried 12 bold", width=10, height=1,bg='lightgreen', command=self.averageAgee2)

        self.search.place(x=160, y=50)
        self.averageage.pack(fill='both', expand=True)
    def averageAgee2(self):
        self.averageage.destroy()
        self.__init__(self.window)
        sql= "SELECT AVG(age) AS AvgAge FROM appointments"
        ...
        self.id = Label(self.averageage, text="Average Age", font=('arial 12 bold'), fg="red")
        self.id.place(x=0, y=12)
        tkinter.messagebox.showinfo("Average Age ", out )
    ...
    def subQuery(self):
        ...
        self.id = Label(self.subqueryframe, text="subquery", font=('arial 12 bold'), fg="red")
        self.id.place(x=0, y=12)
        
        self.search = Button(self.subqueryframe, text="subquery", font="aried 12 bold", width=10, height=1,bg='lightgreen', command=self.subQuery2)

        self.search.place(x=160, y=50)
        self.subqueryframe.pack(fill='both', expand=True)
    def subQuery2(self):
            self.subqueryframe.destroy()
            self.__init__(self.window)

            self.id = Label(self.averageage, text="subquery", font=('arial 12 bold'), fg="red")
            self.id.place(x=0, y=12)
            out2="output2"
            tkinter.messagebox.showinfo("subquery",out2 )
    ...
    def addQuery(self):
        ...
        self.id = Label(self.addqueryframe, text="Count of male and female", font=('arial 12 bold'), fg="red")
        self.id.place(x=0, y=12)
        
        self.search = Button(self.addqueryframe, text="count gender", font="aried 12 bold", width=10, height=1,bg='lightgreen', command=self.addQuery2)

        self.search.place(x=160, y=50)
        self.addqueryframe.pack(fill='both', expand=True)
    def addQuery2(self):
        self.addqueryframe.destroy()
        self.__init__(self.window)
        sql1= "SELECT COUNT(gender) FROM appointments WHERE gender like 'M%';"
        sql2= "SELECT COUNT(gender) FROM appointments WHERE gender like 'F%';"
        self.res = c.execute(sql1)
        ...
        self.id.place(x=0, y=12)
        out3="output3"
        tkinter.messagebox.showinfo("Count of male and female"," male : "+out1+"  female : "+out2 )
...
def menubar():
    ...
    file_menu.add_command(label="average Age", command=b.averageAgee)
    file_menu.add_command(label="Sub Query", command=b.subQuery)
    file_menu.add_command(label="Count of gender", command=b.addQuery)
